        ...
        <div className="grid md:grid-cols-2 gap-4">
          {Object.entries(categories).map(([category, words]) => (
            <button
              ...
              className="bg-gray-900/50 hover:bg-red-900/30 border border-gray-700/50 hover:border-red-600/50 rounded-2xl p-6 transition-all transform hover:scale-105 text-center"
            >
              <h3 className="text-xl font-bold text-white mb-2">{category}</h3>
              <p className="text-gray-400 text-sm">{words.length} words available</p>
            </button>
          ))}
        </div>
      </div>
